...
let pageConfig = {
  ...
  onLoad: function (options) {
    ...
    this.gnotice() // 消息推送官方的相关公告
    this.tempSupport() // 消息中的点赞、评论红点数
    this.selectBycoded() //获取公告列表
  },
  ...
  selectBycoded() {
    ...
    // })
    wx.request({
      url: `${baseUrl}/notice/selectBycoded/0`,
      header: {
        'content-type': 'application/json' // 默认值
      },
      success(res) {
    ...
    wx.request({
      url: `${baseUrl}/notice/selectBycoded/1`,
      header: {
        'content-type': 'application/json' // 默认值
      },
      success(res) {
        that.setData({
          selectBycodedList1: res.data.data
        })
        console.log('kbjsd', res.data.data)
      }
    })
  ...
  tempSupport() {
    ...
    wx.request({
      ...
      success(res) {
        console.log('消息中的点赞、评论红点数', res);
        that.setData({
          lightRed: res.data.data
        })
      }
    })
    // tempSupport({
